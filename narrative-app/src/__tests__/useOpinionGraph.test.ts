...
function setupHookTest(identity = { did: 'did:key:test', displayName: 'Test User' }) {
  ...
  // Initialize document
  handle.change((d) => {
    Object.assign(d, createEmptyOpinionGraphDoc(identity));
  });
...
describe('useOpinionGraph Hook', () => {
  ...
  describe('updateAssumption', () => {
    ...
    it.skip('should update assumption tags using granular operations', async () => {
      ...
      const assumptionId = result.current!.assumptions[0].id;
      const doc = handle.docSync()!;
      const tagIdsBefore = [...doc.data.assumptions[assumptionId].tagIds];
  ...
  describe('deleteAssumption', () => {
    it('should delete an assumption', async () => {
      ...
      const doc = handle.docSync()!;
      expect(doc.data.assumptions[assumptionId]).toBeUndefined();
    });
    ...
    it('should delete associated votes when deleting assumption', async () => {
      ...
      await waitFor(() => {
        const doc = handle.docSync()!;
        expect(Object.keys(doc.data.votes)).toHaveLength(1);
      });

      const voteId = Object.keys(handle.docSync()!.data.votes)[0];
      ...
      await waitFor(() => {
        const doc = handle.docSync()!;
        expect(doc.data.assumptions[assumptionId]).toBeUndefined();
        expect(doc.data.votes[voteId]).toBeUndefined();
      });
    });
  ...
  describe('removeVote', () => {
    it('should remove user vote from assumption', async () => {
      ...
      const doc = handle.docSync()!;
      expect(Object.keys(doc.data.votes)).toHaveLength(0);
    });
