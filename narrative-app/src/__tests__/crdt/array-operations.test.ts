...
describe('CRDT Array Operation Safety', () => {
  describe('tagIds array operations', () => {
    it('should use granular operations when adding tags', () => {
      ...
      handle.change((d) => {
        d.data.assumptions['a1'].tagIds.push('tag1');
        d.data.assumptions['a1'].tagIds.push('tag2');
      });
      ...
      // Verify result
      expect(doc.data.assumptions['a1'].tagIds).toEqual(['tag1', 'tag2']);
    ...
    it.skip('should use splice when removing tags', () => {
      ...
      handle.change((d) => {
        const tagIds = d.data.assumptions['a1'].tagIds;
      ...
      expect(doc.data.assumptions['a1'].tagIds).toEqual(['tag1', 'tag3']);
    });
    ...
    it('should handle tag updates with minimal operations', () => {
      ...
      handle.change((d) => {
        const assumption = d.data.assumptions['a1'];
      ...
      expect(doc.data.assumptions['a1'].tagIds).toContain('tag2');
      expect(doc.data.assumptions['a1'].tagIds).toContain('tag3');
      expect(doc.data.assumptions['a1'].tagIds).toContain('tag4');
      expect(doc.data.assumptions['a1'].tagIds).not.toContain('tag1');
    });
    ...
    it('should NOT use array replacement (anti-pattern)', () => {
      ...
      handle.change((d) => {
        ...
        d.data.assumptions['a1'].tagIds = ['tag3', 'tag4'] as any;
      });
      ...
      expect(doc.data.assumptions['a1'].tagIds).toEqual(['tag3', 'tag4']);
  ...
  describe('voteIds array operations', () => {
    it('should use push when adding votes', () => {
      ...
      handle.change((d) => {
        d.data.assumptions['a1'].voteIds.push('v1');
      });

      const doc = handle.docSync()!;
      expect(doc.data.assumptions['a1'].voteIds).toEqual(['v1']);
    });
    ...
    it.skip('should use splice when removing votes', () => {
      ...
      handle.change((d) => {
        const voteIds = d.data.assumptions['a1'].voteIds;
      ...
      const doc = handle.docSync()!;
      expect(doc.data.assumptions['a1'].voteIds).toEqual(['v1', 'v3']);
    });
  });
  ...
  describe('editLogIds array operations', () => {
    it('should only append to edit log (never remove)', () => {
      ...
      // Append edits
      handle.change((d) => {
        d.data.assumptions['a1'].editLogIds.push('e1');
      });

      handle.change((d) => {
        d.data.assumptions['a1'].editLogIds.push('e2');
      });

      const doc = handle.docSync()!;
      expect(doc.data.assumptions['a1'].editLogIds).toEqual(['e1', 'e2']);
    });
    ...
    it('should preserve edit log order', () => {
      ...
      expect(doc.data.assumptions['a1'].editLogIds[0]).toBe('e1');
      expect(doc.data.assumptions['a1'].editLogIds[1]).toBe('e2');
      expect(doc.data.assumptions['a1'].editLogIds[2]).toBe('e3');
    });
  });
  ...
  describe('Concurrent array modifications', () => {
    it('should merge concurrent tag additions from two peers', () => {
      ...
      const baseDoc = Automerge.from(createEmptyOpinionGraphDoc(identity));
      ...
      expect(merged.data.assumptions['a1'].tagIds).toContain('tag1');
      expect(merged.data.assumptions['a1'].tagIds).toContain('tag2');
    });
    ...
    it('should handle concurrent vote additions', () => {
      ...
      const doc2 = Automerge.change(fork2, (d) => {
        d.data.assumptions['a1'].voteIds.push('v2');
      });

      const merged = Automerge.merge(doc1, doc2);

      expect(merged.data.assumptions['a1'].voteIds).toHaveLength(2);
      expect(merged.data.assumptions['a1'].voteIds).toContain('v1');
      expect(merged.data.assumptions['a1'].voteIds).toContain('v2');
    });
    ...
    it('should handle concurrent edit log entries', () => {
      ...
      expect(merged.data.assumptions['a1'].editLogIds).toHaveLength(2);
      expect(merged.data.assumptions['a1'].editLogIds).toContain('e1');
      expect(merged.data.assumptions['a1'].editLogIds).toContain('e2');
    });
  });
});
