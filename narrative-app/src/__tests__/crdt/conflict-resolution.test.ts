...
describe('CRDT Conflict Resolution', () => {
  describe('Concurrent assumption creation', () => {
    it('should preserve both assumptions when created with different IDs', () => {
      ...
      expect(Object.keys(merged.data.assumptions)).toHaveLength(2);
      expect(merged.data.assumptions['a1']).toBeDefined();
      expect(merged.data.assumptions['a2']).toBeDefined();
      expect(merged.data.assumptions['a1'].sentence).toBe('First assumption');
      expect(merged.data.assumptions['a2'].sentence).toBe('Second assumption');
    });
    ...
    it('should handle last-write-wins when same ID is used', () => {
      ...
      expect(Object.keys(merged.data.assumptions)).toHaveLength(1);
      expect(merged.data.assumptions['a1']).toBeDefined();
  ...
  describe('Concurrent sentence edits', () => {
    it('should resolve conflicting sentence updates', () => {
      ...
      const doc1 = Automerge.change(fork1, (d) => {
        d.data.assumptions['a1'].sentence = 'First edit';
        d.data.assumptions['a1'].updatedAt = 1000;
      });

      const fork2 = Automerge.clone(withAssumption);
      const doc2 = Automerge.change(fork2, (d) => {
        d.data.assumptions['a1'].sentence = 'Second edit';
        d.data.assumptions['a1'].updatedAt = 2000;
      });

      const merged = Automerge.merge(doc1, doc2);

      // Automerge will pick one sentence (last-write-wins by actor ID)
      expect(merged.data.assumptions['a1'].sentence).toBeDefined();
      // The exact winner depends on Automerge's actor IDs
      expect(
        merged.data.assumptions['a1'].sentence === 'First edit' ||
        merged.data.assumptions['a1'].sentence === 'Second edit'
      ).toBe(true);
    });
  });
  ...
  describe('Concurrent vote operations', () => {
    it('should preserve concurrent votes from different users', () => {
      ...
      const doc1 = Automerge.change(fork1, (d) => {
        ...
        d.data.assumptions['a1'].voteIds.push(voteId);
      });
      ...
      expect(Object.keys(merged.data.votes)).toHaveLength(2);
      expect(merged.data.votes['v1']).toBeDefined();
      expect(merged.data.votes['v2']).toBeDefined();
      expect(merged.data.assumptions['a1'].voteIds).toHaveLength(2);
    });
    ...
    it('should handle same user changing vote concurrently', () => {
      ...
      const withVote = Automerge.change(Automerge.from(createEmptyOpinionGraphDoc(identity)), (d) => {
        d.data.assumptions['a1'] = {
          id: 'a1',
          sentence: 'Test',
          createdBy: 'did:key:test',
      ...
      const doc1 = Automerge.change(fork1, (d) => {
        d.data.votes['v1'].value = 'yellow';
        d.data.votes['v1'].updatedAt = 2000;
      });

      // User changes vote to red on device 2 (offline)
      const fork2 = Automerge.clone(withVote);
      const doc2 = Automerge.change(fork2, (d) => {
        d.data.votes['v1'].value = 'red';
        d.data.votes['v1'].updatedAt = 2001;
      });

      const merged = Automerge.merge(doc1, doc2);

      // One vote wins (last-write-wins or actor-based)
      expect(merged.data.votes['v1'].value).toBeDefined();
      expect(['yellow', 'red']).toContain(merged.data.votes['v1'].value);
    });
  });
  ...
  describe('Delete operations', () => {
    it('should handle delete vs edit conflict (delete wins)', () => {
      ...
      const fork1 = Automerge.clone(withAssumption);
      const doc1 = Automerge.change(fork1, (d) => {
        delete d.data.assumptions['a1'];
      });

      // Peer 2 edits assumption concurrently
      const fork2 = Automerge.clone(withAssumption);
      const doc2 = Automerge.change(fork2, (d) => {
        d.data.assumptions['a1'].sentence = 'Updated';
      });

      const merged = Automerge.merge(doc1, doc2);

      // Delete typically wins in CRDT semantics
      expect(merged.data.assumptions['a1']).toBeUndefined();
    });
  ...
  describe('Tag operations', () => {
    it('should merge concurrent tag creations with different names', () => {
      ...
      expect(Object.keys(merged.data.tags)).toHaveLength(2);
      expect(merged.data.tags['t1'].name).toBe('Frontend');
      expect(merged.data.tags['t2'].name).toBe('Backend');
    });
    ...
    it('should handle concurrent tag assignments to assumption', () => {
      ...
      const fork1 = Automerge.clone(withAssumption);
      const doc1 = Automerge.change(fork1, (d) => {
        d.data.assumptions['a1'].tagIds.push('t1');
      });

      // Peer 2 adds t2 to assumption
      const fork2 = Automerge.clone(withAssumption);
      const doc2 = Automerge.change(fork2, (d) => {
        d.data.assumptions['a1'].tagIds.push('t2');
      });

      const merged = Automerge.merge(doc1, doc2);

      // Both tags should be assigned
      expect(merged.data.assumptions['a1'].tagIds).toHaveLength(2);
      expect(merged.data.assumptions['a1'].tagIds).toContain('t1');
      expect(merged.data.assumptions['a1'].tagIds).toContain('t2');
    });
  });
